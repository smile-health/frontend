// TODO: Consider removing this file if it negatively impacts TypeScript server performance
import 'i18next'

import activity from '../../src/pages/activity/locales/en.json'
import assetInventory from '../../src/pages/asset-inventory/locales/en.json'
import asset from '../../src/pages/asset/locales/en.json'
import entity from '../../src/pages/entity/locales/en.json'
import patient from '../../src/pages/global-settings/patient/locales/en.json'
import population from '../../src/pages/global-settings/population/locales/en.json'
import materialVolume from '../../src/pages/material-volume-management/locales/en.json'
import navbar from '../components/navbar/locales/en.json'
import account from '../pages/account/locales/en.json'
import annualCommitmentList from '../pages/annual-commitment/list/locales/en.json'
import annualCommitmentDetail from '../pages/annual-commitment/detail/locales/en.json'
import annualCommitmentForm from '../pages/annual-commitment/form/locales/en.json'
import annualPlanningProcess from '../pages/annual-planning-process/locales/en.json'
import annualPlanningSubstitution from '../pages/annual-planning-substitution/locales/en.json'
import annualPlanningTargetGroup from '../pages/annual-planning-target-group/locales/en.json'
import task from '../pages/annual-planning-task/locales/en.json'
import assetManagements from '../pages/asset-managements/locales/en.json'
import monitoringDeviceInventory from '../pages/asset-managements/monitoring-device-inventory/locales/en.json'
import monitoringDeviceInventoryDetail from '../pages/asset-managements/monitoring-device-inventory/MonitoringDeviceInventoryDetail/locales/en.json'
import monitoringDeviceInventoryForm from '../pages/asset-managements/monitoring-device-inventory/MonitoringDeviceInventoryForm/locales/en.json'
import monitoringDeviceInventoryList from '../pages/asset-managements/monitoring-device-inventory/MonitoringDeviceInventoryList/locales/en.json'
import storageTemperatureMonitoring from '../pages/asset-managements/storage-temperature-monitoring/StorageTemperatureMonitoring/locales/en.json'
import storageTemperatureMonitoringDetail from '../pages/asset-managements/storage-temperature-monitoring/StorageTemperatureMonitoringDetail/locales/en.json'
import storageTemperatureMonitoringList from '../pages/asset-managements/storage-temperature-monitoring/StorageTemperatureMonitoringList/locales/en.json'
import assetType from '../pages/asset-type/locales/en.json'
import assetVendor from '../pages/asset-vendor/locales/en.json'
import budgetSource from '../pages/budget-source/locales/en.json'
import dashboardAsik from '../pages/dashboard/dashboard-asik/locales/en.json'
import dashboardDiscard from '../pages/dashboard/dashboard-discard/locales/en.json'
import dashboardDownload from '../pages/dashboard/dashboard-dowload-file/locales/en.json'
import dashboardInventoryOverview from '../pages/dashboard/dashboard-inventory-overview/locales/en.json'
import dashboardInventory from '../pages/dashboard/dashboard-inventory/locales/en.json'
import dashboardOrder from '../pages/dashboard/dashboard-order/locales/en.json'
import dashboardRabies from '../pages/dashboard/dashboard-rabies/locales/en.json'
import dashboardReport from '../pages/dashboard/dashboard-report/locales/en.json'
import dashboardStockTaking from '../pages/dashboard/dashboard-stock-taking/locales/en.json'
import dashboardStock from '../pages/dashboard/dashboard-stock/locales/en.json'
import dashboardMicroplanning from '../pages/dashboard/dashboard-microplanning/locales/en.json'
import dashboardMonitoringTransactions from '../pages/dashboard/dashboard-transaction-monitoring/locales/en.json'
import dashboard from '../pages/dashboard/locales/en.json'
import disposalInstructionCreate from '../pages/disposal-instruction/DisposalInstructionCreate/locales/en.json'
import disposalInstructionDetail from '../pages/disposal-instruction/DisposalInstructionDetail/locales/en.json'
import disposalInstructionList from '../pages/disposal-instruction/DisposalInstructionList/locales/en.json'
import disposalInstruction from '../pages/disposal-instruction/locales/en.json'
import distributionDisposal from '../pages/disposal/distribution-disposal/locales/en.json'
import disposalList from '../pages/disposal/list-disposal/locales/en.json'
import entityMaterialBulk from '../pages/entity-material-bulk/locales/en.json'
import exportHistory from '../pages/export-history/locales/en.json'
import forgot from '../pages/forgot-password/locales/en.json'
import programGlobalSettings from '../pages/global-settings/program/locales/en.json'
import stock from '../pages/inventory/stock/locales/en.json'
import login from '../pages/login/locales/en.json'
import manufacturer from '../pages/manufacturer/locales/en.json'
import material from '../pages/material/locales/en.json'
import modelAsset from '../pages/model-asset-management/locales/en.json'
import notification from '../pages/notification/locales/en.json'
import order from '../pages/order/locales/en.json'
import orderCreate from '../pages/order/OrderCreate/locales/en.json'
import orderCentralDistribution from '../pages/order/OrderCreateCentralDistribution/locales/en.json'
import orderDistribution from '../pages/order/OrderCreateDistribution/locales/en.json'
import orderCreateRelocation from '../pages/order/orderCreateRelocation/locales/en.json'
import orderCreateReturn from '../pages/order/OrderCreateReturn/locales/en.json'
import orderDetail from '../pages/order/OrderDetail/locales/en.json'
import orderList from '../pages/order/OrderList/locales/en.json'
import periodOfStockTaking from '../pages/period-of-stock-taking/locales/en.json'
import programPlanMaterialRatio from '../pages/program-plan-material-ratio/locales/en.json'
import programPlan from '../pages/program-plan/locales/en.json'
import program from '../pages/program/locales/en.json'
import protocol from '../pages/protocol/locales/en.json'
import reconciliation from '../pages/reconciliation/locales/en.json'
import reconciliationDetail from '../pages/reconciliation/reconciliationDetail/locales/en.json'
import lplpo from '../pages/report/lplpo/locales/en.json'
import reconciliationReport from '../pages/report/reconciliation/locales/en.json'
import stockBook from '../pages/report/stock-book/locales/en.json'
import userActivity from '../pages/report/user-activity/locales/en.json'
import selfDisposal from '../pages/self-disposal/locales/en.json'
import stockOpnameCreate from '../pages/stock-opname/create/locales/en.json'
import stockOpname from '../pages/stock-opname/locales/en.json'
import ticketingSystem from '../pages/ticketing-system/locales/en.json'
import ticketingSystemCreate from '../pages/ticketing-system/TicketingSystemCreate/locales/en.json'
import ticketingSystemDetail from '../pages/ticketing-system/TicketingSystemDetail/locales/en.json'
import ticketingSystemList from '../pages/ticketing-system/TicketingSystemList/locales/en.json'
import transaction from '../pages/transaction/locales/en.json'
import transactionCreate from '../pages/transaction/TransactionCreate/locales/en.json'
import transactionCreateAddStock from '../pages/transaction/TransactionCreate/TransactionAddStock/locales/en.json'
import transactionCreateConsumption from '../pages/transaction/TransactionCreate/TransactionConsumption/locales/en.json'
import transactionList from '../pages/transaction/TransactionList/locales/en.json'
import user from '../pages/user/locales/en.json'
import common from './en/common.json'
import pqs from './en/pqs.json'
import assetVendor from '../pages/asset-vendor/locales/en.json'
import storageTemperatureMonitoring from '../pages/storage-temperature-monitoring/locales/en.json'
import dashboardAssetOwnershipInventory from '../pages/dashboard/dashboard-asset-ownership-inventory/locales/en.json'
import dashboardAssetTemperatureMonitoring from '../pages/dashboard/dashboard-temperature-monitoring/locales/en.json'
import annualCommitmentDetail from '../pages/annual-commitment/detail/locales/en.json'
import coldStorageCapacity from '../pages/cold-storage-capacity/locales/en.json'

declare module 'i18next' {
  interface CustomTypeOptions {
    defaultNS: 'common'
    resources: {
      assetType: typeof assetType
      account: typeof account
      annualCommitmentList: typeof annualCommitmentList
      annualCommitmentDetail: typeof annualCommitmentDetail
      annualCommitmentForm: typeof annualCommitmentForm
      annualPlanningTargetGroup: typeof annualPlanningTargetGroup
      annualPlanningSubstitution: typeof annualPlanningSubstitution
      asset: typeof asset
      assetInventory: typeof assetInventory
      assetVendor: typeof assetVendor
      common: typeof common
      activity: typeof activity
      manufacturer: typeof manufacturer
      user: typeof user
      entity: typeof entity
      material: typeof material
      materialVolume: typeof materialVolume
      navbar: typeof navbar
      login: typeof login
      budgetSource: typeof budgetSource
      entityMaterialBulk: typeof entityMaterialBulk
      transaction: typeof transaction
      transactionCreate: typeof transactionCreate
      transactionList: typeof transactionList
      forgot: typeof forgot
      program: typeof program
      order: typeof order
      pqs: typeof pqs
      orderList: typeof orderList
      orderDetail: typeof orderDetail
      orderCreate: typeof orderCreate
      orderDistribution: typeof orderDistribution
      orderCreateReturn: typeof orderCreateReturn
      orderCentralDistribution: typeof orderCentralDistribution
      stock: typeof stock
      transactionCreateAddStock: typeof transactionCreateAddStock
      stockOpname: typeof stockOpname
      stockOpnameCreate: typeof stockOpnameCreate
      ticketingSystem: typeof ticketingSystem
      ticketingSystemList: typeof ticketingSystemList
      ticketingSystemCreate: typeof ticketingSystemCreate
      ticketingSystemDetail: typeof ticketingSystemDetail
      reconciliation: typeof reconciliation
      reconciliationDetail: typeof reconciliationDetail
      reconciliationReport: typeof reconciliationReport
      transactionCreateConsumption: typeof transactionCreateConsumption
      programGlobalSettings: typeof programGlobalSettings
      programPlanMaterialRatio: typeof programPlanMaterialRatio
      disposalList: typeof disposalList
      selfDisposal: typeof selfDisposal
      disposalInstruction: typeof disposalInstruction
      disposalInstructionList: typeof disposalInstructionList
      disposalInstructionCreate: typeof disposalInstructionCreate
      disposalInstructionDetail: typeof disposalInstructionDetail
      programPlan: typeof programPlan
      notification: typeof notification
      distributionDisposal: typeof distributionDisposal
      dashboard: typeof dashboard
      dashboardDownload: typeof dashboardDownload
      dashboardMicroplanning: typeof dashboardMicroplanning
      dashboardMonitoringTransactions: typeof dashboardMonitoringTransactions
      dashboardStock: typeof dashboardStock
      dashboardStockTaking: typeof dashboardStockTaking
      dashboardOrder: typeof dashboardOrder
      dashboardInventory: typeof dashboardInventory
      dashboardDiscard: typeof dashboardDiscard
      dashboardReport: typeof dashboardReport
      dashboardAsik: typeof dashboardAsik
      dashboardInventoryOverview: typeof dashboardInventoryOverview
      dashboardRabies: typeof dashboardRabies
      periodOfStockTaking: typeof periodOfStockTaking
      assetManagements: typeof assetManagements
      modelAsset: typeof modelAsset
      userActivity: typeof userActivity
      stockBook: typeof stockBook
      exportHistory: typeof exportHistory
      orderCreateRelocation: typeof orderCreateRelocation
      monitoringDeviceInventory: typeof monitoringDeviceInventory
      monitoringDeviceInventoryDetail: typeof monitoringDeviceInventoryDetail
      monitoringDeviceInventoryForm: typeof monitoringDeviceInventoryForm
      monitoringDeviceInventoryList: typeof monitoringDeviceInventoryList
      storageTemperatureMonitoring: typeof storageTemperatureMonitoring
      storageTemperatureMonitoringDetail: typeof storageTemperatureMonitoringDetail
      storageTemperatureMonitoringList: typeof storageTemperatureMonitoringList
      protocol: typeof protocol
      patient: typeof patient
      annualPlanningProcess: typeof annualPlanningProcess
      coldStorageCapacity: typeof coldStorageCapacity
      population: typeof population
      task: typeof task
      lplpo: typeof lplpo
      dashboardAssetOwnershipInventory: typeof dashboardAssetOwnershipInventory
      dashboardAssetTemperatureMonitoring: typeof dashboardAssetTemperatureMonitoring
      annualCommitmentDetail: typeof annualCommitmentDetail
    }
  }
}
