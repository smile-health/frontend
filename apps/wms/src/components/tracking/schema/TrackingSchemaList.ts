...
export const createFilterTrackingGroupSchema = ({
  t,
  userRole,
}: Params): UseFilter => {
  const filters: UseFilter = [
    {
      ...
      name: 'search',
      id: 'input-search',
      label: t('tracking:list.filter.search.label'),
      placeholder: t('tracking:list.filter.search.placeholder'),
      className: 'ui-w-full',
      defaultValue: '',
      maxLength: 255,
    },
    {
      ...
      multicalendar: true,
      clearable: false,
      label: t('tracking:list.filter.date_range.label'),
      defaultValue: {
    ...
    },
    {
      ...
      className: 'ui-w-full',
      isMulti: false,
      label: t('tracking:list.filter.healthcare_facility_name.label'),
      placeholder: t(
        'tracking:list.filter.healthcare_facility_name.placeholder'
      ),
      ...
      disabled: ({ getValue }) =>
        !getValue('dateRange') || !!getDefaultHealthcareFacilityValue(),
    },
  ];
